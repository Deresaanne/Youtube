...
class CommandParser:
    ...
    def execute_command(self, command: Sequence[str]):
        ...
        elif command[0].upper() == "SHOW_CURRENT_PLAYLISTS":
            self._player.show_current_playlists()
        ...
        elif command[0].upper() == "UNDO_VIDEO":
            if len(command) != 2:
                raise CommandException(
                    "Please enter UNDO command followed by a "
                    "video_id"
                )
            self._player.undo_video(command[1])
    ...
    def _get_help(self):
        ...
        Available commands:
            ...
            PLAY_PLAYLIST <playlist_name> - Play existing playlists
            SHOW_CURRENT_PLAYLISTS - Showing playlists which are currently playing
            RATE_VIDEO <video_id> <rating> - Rate the videos and show a list ordered by their rating
            UNDO_VIDEO <video_id> Undo existing rating so that users can re-enter the rating again.
            HELP - Displays help.
